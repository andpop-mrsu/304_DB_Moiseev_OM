...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($firstName === '' || $lastName === '' || $specialization === '') {
        ...
        $stmt = $db->prepare('INSERT INTO employees (first_name, last_name, specialization) VALUES (?, ?, ?)');
        $stmt->execute([$firstName, $lastName, $specialization]);
        header('Location: index.php?message=Мастер успешно добавлен');
        exit;
    }
}
?>
